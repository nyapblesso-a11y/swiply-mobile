...
export default function DocumentPreviewScreen() {
  ...
  function latestDoc(type: "cv" | "cover_letter") {
    const matches =
      data?.generatedDocuments.filter((d) => d.type === type) ?? [];
    return matches.sort(
      (a, b) =>
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        <Pressable
          ...
          hitSlop={12}
        >
          <Text style={[styles.backArrow, { color: "green", fontSize: 40 }]}>
            ←
          </Text>
        </Pressable>
        <View style={{ flex: 1 }}>
      ...
      <View style={styles.tabRow}>
        ...
        >
          <Text
            style={[styles.tabText, activeTab === "cv" && styles.tabTextActive]}
          >
            CV
          </Text>
        </Pressable>
      ...
      <View style={styles.bottomBar}>
        <Pressable
          ...
          onPress={() =>
            downloadDoc(activeDoc, activeTab === "cv" ? "CV" : "Cover_Letter")
          }
        >
          {downloading ? (
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 17, fontWeight: "600", color: NAVY, marginTop: 10, textAlign: "center"  },
  headerSubtitle: { fontSize: 12, color: GRAY, marginTop: 6, textAlign: "center" },
  tabRow: {
    flexDirection: "row",
    marginHorizontal: 20,
